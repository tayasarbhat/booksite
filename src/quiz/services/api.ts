...
const cache = {
  ...
  CACHE_DURATION: 10 * 60 * 1000 // Increased from 5 to 10 minutes
};
...
export async function fetchQuestions(subject: string) {
  try {
    ...
    if (questions && Array.isArray(questions) && questions.length > 0) {
      ...
      // Preload only one other subject in background to reduce load
      if (subjects.length > 1) {
        const otherSubject = subjects.find(s => s.id !== subject);
        if (otherSubject) {
          setTimeout(() => {
            preloadQuestions(otherSubject.id).catch(() => {
              // Silently handle background preloading errors
            });
          }, 5000); // Delay preloading by 5 seconds
        }
      }
    }
...
export async function fetchAllSubjects() {
  try {
    ...
    if (data.subjects && Array.isArray(data.subjects)) {
      ...
      // Preload first subject's questions if available, with delay
